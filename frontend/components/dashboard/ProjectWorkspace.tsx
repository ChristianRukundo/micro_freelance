    ...
    return (
      <div className="space-y-4">
        ...
        {Array.from({ length: 3 }).map((_, i) => (
          <Card
            key={i}
            className="rounded-xl border border-neutral-200 bg-card shadow-soft dark:shadow-soft-dark p-4 space-y-2"
          >
  ...
  if (isErrorMilestones)
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Failed to load milestones: {errorMilestones?.message}
        </AlertDescription>
      </Alert>
    );
...
function MilestoneCard({
  ...
  const [isRevisionDialogValid, setIsRevisionDialogValid] =
    ...
    <Card className="rounded-xl border border-neutral-200 bg-card shadow-soft dark:shadow-soft-dark p-4 space-y-3">
      ...
      {milestone.status === MilestoneStatus.REVISION_REQUESTED &&
        milestone.comments && (
          <Alert
            variant="destructive"
            className="ml-7 bg-error-50 text-error-700 border-error-200"
          >
