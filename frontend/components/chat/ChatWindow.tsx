...
export function ChatWindow({ taskId }: ChatWindowProps) {
  ...
  return (
    <Card className="h-[500px] flex flex-col shadow-medium dark:shadow-medium-dark border-neutral-200">
      <CardContent className="flex-1 overflow-hidden p-0">
        <ScrollArea className="h-full">
          <div className="flex flex-col gap-4 p-4 pb-0">
            {isFetchingNextPage && (
              <div className="flex justify-center p-2">
                <LoadingSpinner size="sm" className="mr-2" /> Loading old
                messages...
              </div>
            )}
...
function ChatMessage({ message, isCurrentUser }: ChatMessageProps) {
  const avatarSeed =
    message.sender?.profile?.firstName || message.sender?.email || "User";
    ...
    >
      ...
      >
        <p className="font-medium text-caption mb-1">
          {message.sender?.profile?.firstName || "Anonymous"}
        </p>
        <p>{message.content}</p>
        <span
          className={cn(
