...
function CheckoutForm({ taskId, taskBudget, onPaymentSuccess }: PaymentFormProps) {
  ...
  const { mutateAsync: createPaymentIntentMutation, isPending: isCreatingIntent } = useMutation({
    mutationFn: (amount: number) => actions.createPaymentIntentAction(taskId, { amount }),
    onSuccess: (response) => {
  ...
  }, [taskId, taskBudget, clientSecret, isCreatingIntent, createPaymentIntentMutation]);
  ...
  const isFormDisabled = isCreatingIntent || isProcessing || !stripe || !elements || !clientSecret;
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {isCreatingIntent ? (
          <div className="flex items-center justify-center p-6 text-body-md text-neutral-600">
            <LoadingSpinner size="md" className="mr-3" /> Initializing payment...
          </div>
          ...
          >
            <FormItem>
              ...
              <div className="rounded-lg border border-neutral-200 p-3 shadow-soft">
                <PaymentElement options={{ layout: 'tabs' }} />
              </div>
              <FormMessage />
            </FormItem>

            <Button type="submit" className="w-full text-body-md shadow-primary group" disabled={isFormDisabled}>
              {isProcessing && <LoadingSpinner size="sm" color="text-primary-foreground" className="mr-2" />}
          ...
          <div className="flex flex-col items-center justify-center p-6 text-body-md text-error-600">
            ...
            <Button onClick={() => createPaymentIntentMutation(form.getValues('amount'))} className="mt-4">Retry</Button>
          </div>
        )}

        {message && (
          <div className={`text-body-sm text-center ${message.includes('succeeded') ? 'text-success-600' : 'text-destructive-500'}`}>
            {message}
          </div>
        )}
      </form>
...
export function PaymentForm({ taskId, taskBudget, onPaymentSuccess }: PaymentFormProps) {
    ...
    >
      <Card className="shadow-medium border-neutral-200">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-h3 font-bold text-neutral-800">Secure Payment</CardTitle>
        ...
        <CardContent>
          {!stripePromise && (
            <div className="flex items-center justify-center p-6 text-body-md text-destructive-500">
              <AlertCircleIcon className="h-6 w-6 mr-2" /> Stripe is not configured. Missing NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY.
            </div>
          )}
          {stripePromise && (
            <Elements options={{ clientSecret: '...', appearance }} stripe={stripePromise}>
              {/* clientSecret is initialized as '...' as it's fetched dynamically by CheckoutForm */}
              <CheckoutForm taskId={taskId} taskBudget={taskBudget} onPaymentSuccess={onPaymentSuccess} />
            </Elements>
          )}
        </CardContent>
      </Card>
