...
export function TaskForms({ formType, initialData, taskId }: TaskFormsProps) {
  ...
  const handleRemoveFile = (
    ...
    form.setValue(
      "attachments",
      (form.getValues("attachments") || []).filter(
        (att: any) => att.url !== url
      )
    );
    if (s3Key) {
    ...
    mutationFn: async (data: TaskFormInput) => {
      ...
      const dataToSend = {
        ...
        deadline: (data as any).deadline
          ? new Date((data as any).deadline).toISOString()
          : "",
      };
      if (formType === "new") {
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="e.g., Develop a landing page for new SaaS"
                      {...field}
        ...
        {currentStep === 2 && (
          ...
          >
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      ...
                      className="min-h-[150px] shadow-soft dark:shadow-soft-dark"
                      {...field}
                      disabled={isFormPending}
                    />
                  </FormControl>
              ...
              >
                ...
                {isUploading ? (
                  <div className="flex items-center text-body-md text-primary-600">
                    ...
                    files ({Math.round(isUploading ? 0 : 0)}%)
                  </div>
                ) : isDragActive ? (
                  <p className="text-body-md text-primary-600">
                    Drop the files here ...
                  </p>
                ) : (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        {fileItem.status === "completed" && (
                          <CheckCircle2Icon className="ml-2 h-4 w-4 text-success-600" />
                        )}
                        {fileItem.status === "failed" && (
                          <XIcon className="ml-2 h-4 w-4 text-error-500" />
                        )}
                      </div>
                      <Button
                        type="button"
            ...
            <div className="flex justify-between mt-8">
              ...
              <Button
                type="submit"
                className="text-body-md shadow-primary dark:shadow-primary-dark group"
                disabled={isFormPending}
              >
                {isFormPending && (
                  <LoadingSpinner
                ...
                {formType === "new" ? "Post Project" : "Save Changes"}{" "}
                <CheckCircle2Icon className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Button>
            </div>
          </motion.div>
        )}
