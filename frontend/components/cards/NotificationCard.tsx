...
const NotificationIcon = ({ type }: { type: NotificationType }) => {
  switch (type) {
    case NotificationType.NEW_BID:
      return <MessageSquareTextIcon className="h-5 w-5 text-primary-500" />;
    case NotificationType.BID_ACCEPTED:
      return <CheckCircle2Icon className="h-5 w-5 text-success-500" />;
    ...
    case NotificationType.MILESTONE_SUBMITTED:
      return <BellIcon className="h-5 w-5 text-warning-500" />;
    case NotificationType.MILESTONE_APPROVED:
      return <BadgeCheckIcon className="h-5 w-5 text-success-500" />;
    ...
    case NotificationType.TASK_CANCELLED:
      return <XCircleIcon className="h-5 w-5 text-destructive-500" />;
    case NotificationType.PAYMENT_SUCCEEDED:
      return <DollarSignIcon className="h-5 w-5 text-success-500" />;
...
export function NotificationCard({ notification }: NotificationCardProps) {
  const { markNotificationAsRead, isMarkingNotificationAsRead } =
    useNotifications();
  ...
  return (
    ...
    >
      <Card
        className={cn(
          "w-full rounded-xl border border-neutral-200 bg-card shadow-soft dark:shadow-soft-dark transition-all duration-300 ease-in-out-quad",
          !notification.isRead
