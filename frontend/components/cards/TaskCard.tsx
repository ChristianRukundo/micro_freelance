...
import { DollarSignIcon, ClockIcon, UsersIcon, FileTextIcon, MoveRightIcon } from 'lucide-react';
...
export function TaskCard({ task, showApplyButton = true }: TaskCardProps) {
  const timeSincePost = formatDistanceToNowStrict(new Date(task.createdAt), { addSuffix: true });
  ...
  return (
    ...
    >
      <Card className="group relative overflow-hidden rounded-xl border border-neutral-200 bg-card shadow-soft transition-all duration-300 ease-in-out-quad hover:shadow-card-hover hover:border-primary-200">
        <Link href={`/tasks/${task.id}`} className="absolute inset-0 z-10" aria-label={`View task ${task.title}`}></Link>
        <CardHeader className="flex flex-row items-start justify-between space-y-0 p-6">
          <div className="flex-1 space-y-2">
            ...
            <Badge variant="secondary" className="text-body-sm font-medium bg-primary-50 text-primary-600">
              {task.category?.name || 'Uncategorized'}
            </Badge>
          </div>
          <div className="relative z-20 flex-shrink-0">
            <Badge variant="outline" className={`font-semibold ${
              task.status === 'OPEN' ? 'bg-success-50 text-success-600 border-success-200' :
              task.status === 'IN_PROGRESS' ? 'bg-warning-50 text-warning-600 border-warning-200' :
              task.status === 'COMPLETED' ? 'bg-neutral-100 text-neutral-600 border-neutral-300' :
            ...
            <div className="flex items-center space-x-2">
              {task.client?.profile?.avatarUrl ? (
                <Avatar className="h-6 w-6">
                  <AvatarImage src={task.client.profile.avatarUrl} alt={task.client.profile.firstName || task.client.email} />
                  <AvatarFallback>{avatarSeed.charAt(0).toUpperCase()}</AvatarFallback>
                </Avatar>
              ) : (
                <Avatar className="h-6 w-6">
                  <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${avatarSeed}`} alt={avatarSeed} />
                  <AvatarFallback>{avatarSeed.charAt(0).toUpperCase()}</AvatarFallback>
                </Avatar>
              )}
              <span>Posted by {task.client?.profile?.firstName || 'Client'}</span>
            </div>
        ...
        <CardFooter className="flex items-center justify-between p-6 pt-0 relative z-20">
          ...
          {showApplyButton && task.status === 'OPEN' && (
            <Link href={`/tasks/${task.id}#bid`} passHref>
              <Button size="sm" className="group text-body-sm shadow-soft hover:shadow-medium">
                Apply Now <MoveRightIcon className="ml-1 h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Button>
            </Link>
          )}
        </CardFooter>
