...
export default function FreelancerDiscoveryPage() {
  const [filters, setFilters] = React.useState<FreelancerQueryFilters>({ sortBy: 'createdAt', sortOrder: 'desc' });
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const params = { ...filters, q: filters.q || undefined, role: UserRole.FREELANCER, page: pageParam, limit: 10 };
  ...
  const handleFilterChange = (key: keyof FreelancerQueryFilters, value: any) => {
  ...
  const isEmpty = !isLoading && !isFetchingNextPage && allFreelancers.length === 0;

  return (
    <div className="container py-8">
      <h1 className="text-display-md font-extrabold mb-8 text-neutral-800">Discover Talented Freelancers</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {isLoading && Array.from({ length: 6 }).map((_, i) => <FreelancerCardSkeleton key={i} />)}
      ...
      {isEmpty && (
        <div className="flex flex-col items-center justify-center py-12 text-center text-neutral-500">
          ...
          <h2 className="text-h3">No freelancers found matching your criteria.</h2>
